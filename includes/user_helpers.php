...
function create_thumbnail(string $srcPath, string $destPath, int $w, int $h, string $mime) {
    ...
    switch ($mime) {
        ...
        case 'image/webp':
            ...
            else $srcImg = imagecreatefromjpeg($srcPath); // fallback
            break;
        default:
            return;
    }
    ...
    switch ($mime) {
        case 'image/jpeg': imagejpeg($dstImg, $destPath, 85); break;
        case 'image/png': imagepng($dstImg, $destPath); break;
        case 'image/gif': imagegif($dstImg, $destPath); break;
        case 'image/webp': if (function_exists('imagewebp')) imagewebp($dstImg, $destPath); else imagejpeg($dstImg, $destPath, 85); break;
    }
    imagedestroy($srcImg);
    imagedestroy($dstImg);
}
